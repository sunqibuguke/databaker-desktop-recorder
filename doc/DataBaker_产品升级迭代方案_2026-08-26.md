...
- 右侧栏拆分“检测”和“设置”：检测只保留判定方式、静音阈值和时长；设置按连续录制、录制保护、提示反馈分组，并明确两个连续录制开关的从属关系；
- 侧栏规则仅保存到当前任务，顶部“应用设置”才修改以后新任务的默认值；恢复工作区时保留最后打开的设置页，不把任务调整静默写成工作站默认；
...
| 驱动上报输入不连续但 `missing_frames=0` | 本次 attempt 标记 `needs_rerecord`，任务继续采集 | discontinuity count 增加、silence samples 不变、异常句不可确认或交付 |
| 活动 attempt 发生真实缺帧 | 持续母音频不中断；本次 attempt 标记 `needs_rerecord` 且不可交付，若有旧版则保留旧版；界面先切到下一物理句并按自动续录/标签规则继续 | 缺帧窗口、attempt 状态、下一句索引与录音状态、旧版关系 |
...
**AC-R04 零缺帧 warning 仍需重录｜首次必过：迭代一回归**

- Given 驱动上报 `DATA_DISCONTINUITY`，但 packet position 连续且 `missing_frames=0`，
- When 操作员完成当前句，
- Then 当前 attempt 进入「需重录」且不可确认或交付；warning 可见，后续任务仍可继续采集。

**AC-R05 真实缺帧继续采集但不交付｜首次必过：迭代一回归**

- Given 活动 attempt 内检测到 `missing_frames>0` 的可恢复缺帧，
- When 操作员完成当前句，
- Then 母音频与整批采集不中断；当前 attempt 持久化为 `needs_rerecord`、禁止确认/试听切片/交付，并保留在问题队列；界面切到下一物理句，按 `autoStartNext` 与标签暂停规则决定是否立即开始。若当前是末句，则停在末句提示重录。
...
| G05 | 迭代一 | 输入不连续：无论是否估算出缺帧均标需重录但任务继续采集；有旧版保留旧版；无活动句不污染 | 测试态 discontinuity 协议事件 + `missing_frames`/silence samples fixture |
