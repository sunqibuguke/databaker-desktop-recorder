...
| 驱动上报输入不连续但 `missing_frames=0` | 只记 warning，本次 attempt 仍可正常确认 | discontinuity count 增加、silence samples 不变、确认与下一句流程正常 |
...
**AC-R04 零缺帧 warning 正常确认｜首次必过：迭代一回归**

- Given 驱动上报 `DATA_DISCONTINUITY`，但 packet position 连续且 `missing_frames=0`，
- When 操作员完成并确认当前句，
- Then 当前 attempt 保持可交付，不进入「需重录」；warning 可见，确认与下一句自动续录流程保持正常。
...
| G05 | 迭代一 | 输入不连续：零缺帧 warning 正常确认；真实缺帧标需重录但切下一句继续采集；有旧版保留旧版；无活动句不污染 | 测试态 discontinuity 协议事件 + `missing_frames`/silence samples fixture |
