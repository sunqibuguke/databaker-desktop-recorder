...
def draw_workflow() -> Path:
    ...
    notes = [
        ...
        ("重录不覆盖", "采录人员只需使用或放弃本次重录。底层会追加保存录音，用于异常恢复和交付校验。"),
...
def draw_params() -> Path:
    im, draw = card(1600, 780, "推荐采集参数", "以设备真实能开的格式为准，不要选界面上看得到但驱动不支持的组合")
    rows = [
        ("常规高清语音", "48,000 Hz", "24-bit PCM", "Mono", "大多数朗读语料的默认选择"),
        ("后期余量大", "48,000 Hz", "32-bit Float", "Mono", "后续处理需要更大动态余量时"),
        ("兼容旧链路", "48,000 Hz 或 44,100 Hz", "16-bit PCM", "Mono", "对接只能吃 16-bit 的下游系统"),
    ]
    heads = ["场景", "采样率", "采集 / 交付格式", "声道", "说明"]
    ...
    notes = [
        "Windows 工位默认 WASAPI 独占开流，绕过系统混音器。独占失败会明确报错，不会悄悄改成共享还显示独占。",
        "系统混音下列出的采集格式是 Windows 接受的客户端格式，不证明声卡 ADC 以同样的原生位深工作。",
        "多输入声卡显示“输入 1、输入 2…”。软件从所选硬件通道采集，交付始终是单声道 WAV。",
        "仅提供 ASIO、且没有可用 Windows 输入端点的声卡不在当前基线内。",
    ]
    for i, n in enumerate(notes):
        draw.ellipse((60, 478 + i * 58, 76, 494 + i * 58), fill=ACCENT)
        for j, line in enumerate(wrap_text(draw, n, body, 1450)):
            draw.text((92, 470 + i * 58 + j * 28), line, font=body, fill=INK2)
    out = FIG / "params.png"
    im.save(out, "PNG")
    return out
