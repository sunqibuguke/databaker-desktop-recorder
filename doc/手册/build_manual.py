...
def build_story() -> list:
    ...
    s.append(P("声卡链路 warning 分两类：驱动仅报告短暂抖动、实际没有缺帧时，本句仍可正常确认并继续下一句；确认到真实缺帧时，本次录音自动标为「需重录」且不能交付，界面先切到下一物理句，并按自动续录与标签暂停规则决定是否立即开始。母音频和整批采集不中断，受影响句保留在「问题」面板，稍后再补录。末句发生真实缺帧时停在末句提示重录。"))
    ...
    s.append(table(
        ...
        [
            ...
            ["输入过载", "降低声卡增益，本句重录"],
            ["几乎无声", "检查通道、接线、系统静音"],
            ["链路 warning（无缺帧）", "本句仍可正常确认，并按规则继续下一句"],
