...
def build_story() -> list:
    ...
    s += [
        heading(1, "新建录制"),
        P("在任务列表点「新建录制」。分三步：导入脚本、选择采集预设/输入设备/通道、命名并选择保存位置。采样率和驱动输入格式在高级设置中复核。创建后进入查看，此时声卡未打开。"),
    ]
    s.extend(figure(
        ANN / "setup.png",
        "新建录制",
        [
            ...
            "采集预设。",
            "输入设备。",
            "输入通道与实际配置摘要；采样率和驱动输入格式收在高级设置。不支持的组合会直接列出原因。",
    ...
    s.append(bullets([
        "采集预设：保存或套用本机常用的设备、通道与驱动输入组合。",
        "Windows 采集模式：独占，或系统混音。独占失败会报错，不会改成混音后仍显示独占。",
        "交付格式：本次试运行的新任务统一为 48 kHz / 16-bit PCM / 单声道，母带、试听和导出保持一致。",
        "驱动输入格式：设备可以实际使用 i16 / i24 / i32 / f32；软件会转换为交付格式，两者会分开记录。",
    ...
    s.append(P("查看时主按钮为「试听本句」，整页右下角为「进入录制」，其下为「退出查看」。点「进入录制」或任务列表的「录制」后打开声卡并开始写母轨。新建或恢复开卡后，先按任务规则做约 3 秒环境噪声检测，再在第一句前完成 10 秒输入试听。输入试听允许明确跳过，并且与环境检测互不替代；同一次应用启动、相同采集配置不重复提示。未进入录制时声卡未打开，查看态显示该句波形或「本句尚无录音」，不把 Peak / RMS 显示成 −∞。"))
    s.append(PageBreak())
