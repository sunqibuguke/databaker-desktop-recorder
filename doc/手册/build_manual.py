...
def draw_cover(canv, doc) -> None:
    ...
    canv.drawRightString(PAGE_W - 22 * mm, 20 * mm, "文档版本 2026-09-08")
    canv.restoreState()
...
def recording_policy_content(kind: str) -> list:
    """Shared operator guidance used by both bundled manuals."""
    if kind == "settings":
        return [
            *figure(CAP / "14-new-recording-policy.png", "新建录制：人声检查与短句录制（界面示例）", max_h=64 * mm),
            P("新建录制和已有任务右侧「设置」均可配置「人声幅值检查」和「短句自动结束」。两个开关相互独立，默认关闭；可只开其中一个。"),
            table("参数与生效范围", ["设置", "说明"], [
                ["人声峰值下限", "默认 3000 samp；本项目的人声绝对峰值下限"],
                ["人声峰值上限", "默认 20000 samp；本项目的人声绝对峰值上限"],
                ["阈值调整", "适配 16-bit PCM；1～32766 整数 samp，下限须小于上限。可切换 dBFS，判定阈值保持不变"],
                ["短句静音时长", "沿用任务的静音时长：0.2～5 秒，默认 1 秒；点击「调整静音时长」进入检测页"],
                ["保存录制设置", "已有任务点保存后，从下一次开始录制本句生效；本次录制沿用开始时的配置"],
            ], [40 * mm, CONTENT_W - 40 * mm]),
            callout("note", "正在录制时修改", "短句模式下，相关静音参数的修改也从下一次录制生效。历史录音保留原检测结果；旧任务保留原 RMS／PEAK 检查，可在设置中主动改用 16-bit 峰值检查；不补算历史。"),
            P("恢复设置会恢复进入任务时的录制策略与录制规则；静音参数使用检测页自己的恢复按钮。保存后核对界面显示，再开始下一次录制。"),
        ]
    if kind == "amplitude":
        return [
            *figure(CAP / "18-warning-confirmation.png", "幅值异常待确认操作区（模拟演示）", max_h=40 * mm),
            P("开启检查后，软件按实际保存的单声道音频中被当前检测器识别为人声的区间计算，排除非人声区间。取人声正负采样值的最大绝对值，不是峰峰值。直接比较保存的 16 位整数采样值，界面平滑电平不用于判定。"),
            table("提醒怎么判断", ["阶段", "含义"], [
                ["本句结束后偏小", "整句人声绝对峰值低于下限时提示；录制中不因开头轻声提前报警"],
                ["录制中声音过大", "人声峰值超过上限立即提示；等于阈值不触发超限"],
                ["结束后检查", "计算整句人声绝对峰值；3000 与 20000 samp 均算达标。短促人声也执行最终检查"],
                ["提醒保留", "录制中发生过的超限提醒随录音保留；旧版结果保留当时的提醒"],
            ], [42 * mm, CONTENT_W - 42 * mm]),
            P("「声音偏小」「声音过大」以黄色图标和文字显示在正文上方固定提示区，并同步到领读面板；不弹窗、不发提示音、不挤动正文。结束后显示异常原因及本次阈值，正常录音仅显示指标。可先试听或重录；核对可用后，点击「确认保留并录下一句」。末句或关闭连续录制时，以「确认保留本句」等当前按钮文案为准。"),
            callout("warn", "先分清提醒和阻断", "人工保留记录随录音保存，重启后仍在；幅值提醒本身不阻止导出。音频中断、写盘失败或检测失败等强制补录问题仍须处理，不能通过确认保留绕过。"),
            P("其他项目如需 5000～25000 samp，请手动调整并保存；25000 是参考上限，不是截幅点。与 AU 核对时，应使用同一段 16-bit 音频及相同人声范围。无有效人声时沿用无声处理。检测器不识别朗读内容，噪声也可能被误判为人声；请结合试听和现场环境复核，不要只凭一个数值确认录音。"),
        ]
    if kind == "auto":
        return [
            *figure(CAP / "16-prompter-speech-warning.png", "自动结束后领读面板等待确认；固定区域同步幅值提醒", max_h=73 * mm),
            table("短句录制流程", ["步骤", "操作与结果"], [
                ["1 开始本句", "按原流程开始并朗读；没有有效人声时不会自动生成空录音"],
                ["2 读完保持安静", "检测到有效人声后，连续静音达到任务设定时长，自动结束本句；达到之前再次发声会重新累计静音"],
                ["3 停在待确认", "系统锁定本句终点并保存，终点后的声音不再进入本句切片；母音频仍持续保存"],
                ["4 人工确认", "先试听、重录或确认；确认后才按连续录制和标签暂停规则进入下一句，自动结束不会自动确认"],
            ], [37 * mm, CONTENT_W - 37 * mm]),
            callout("note", "不能判断有没有多读", "停顿短于设定时长，或连续不停顿的多读，仍可能进入本句。本功能不识别内容、不判断重复次数，也不增加最长时长提醒。"),
            P("「完成本句」和 Space 仍可手动结束。重录保留历史版本；已有合格版本不会因自动结束而被覆盖，须明确选择使用本次重录。"),
        ]
...
def build_story() -> list:
    ...
    s.append(P("录制中授权失效时，立即停止朗读。软件会先安全停止并封存已写入音频，再进入激活页；等待期间不要拔盘或退出。封存失败时按提示重试，处理完成前不能继续录制。激活页还会列出待封存任务；目录或进度异常时须先核对原保存位置并处理提示。"))
    ...
    s.append(table(
        ...
        [
            ...
            ["朗读", "检测到语音后进入录制"],
            ["读完后安静", "累计句尾静音；开启短句自动结束时，达到设定时长即进入待确认"],
            ["空格或「完成本句」", "封闭本句，进入待确认"],
            ["空格确认", "本句标为已确认，转到下一句"],
    ...
    s.append(P("检测页只修改当前任务，不改采集预设。普通模式下按原规则生效；短句自动结束模式下，本次录制固定开始时的静音配置，修改从下一次录制生效。"))
    ...
    for title, kind in [("人声检查与短句设置", "settings"), ("人声幅值提醒与人工保留", "amplitude"), ("短句自动结束", "auto")]:
        s.append(PageBreak())
        s.append(heading(1, title))
        s.extend(recording_policy_content(kind))
    s.append(PageBreak())
    ...
    s.append(P("人声指标、幅值提醒、当次检测配置、人工保留与结束原因随录音和导出元数据保存。历史中缺少新检测结果表示当时未检查，不代表达标。幅值异常须先人工确认；阻断性问题仍按原规则处理。外部设备 WAV 的切分操作另见《外部设备WAV切分脚本使用说明》，应使用 timestamps.json，不能使用导出状态文件代替。"))
    s.append(table(
        "导出内容",
        ["项", "文件", "内容"],
        [
            ["整轨 WAV", "full-track.wav", "连续母轨"],
            ["时间戳 JSON", "timestamps.json", "句子文本、选中录音、样本起止及检测记录"],
    ...
    s.append(P("标贝音频采集 0.2  ·  2026-09-08", "center_muted"))
    return s
