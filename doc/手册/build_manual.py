...
def draw_cover(canv, doc) -> None:
    ...
    canv.drawRightString(PAGE_W - 22 * mm, 20 * mm, "文档版本 2026-09-07")
    canv.restoreState()
    ...
    if kind == "settings":
        return [
            *figure(CAP / "14-new-recording-policy.png", "新建录制：人声检查与短句录制（界面示例）", max_h=64 * mm),
            ...
            table("参数与生效范围", ["设置", "说明"], [
                ["人声 RMS 下限", "初始参考值 −30 dBFS；整句人声能量的下限"],
                ["人声 PEAK 上限", "初始参考值 −3 dBFS；人声最大峰值的上限"],
                ["阈值调整", "可在 −96～0 dBFS 内调整，下限须小于上限。两项是不同指标；参考值不是验收标准，应按项目要求试录确定"],
                ["短句静音时长", "沿用任务的静音时长：0.2～5 秒，默认 1 秒；点击「调整静音时长」进入检测页"],
                ["保存录制设置", "已有任务点保存后，从下一次开始录制本句生效；本次录制沿用开始时的配置"],
            ], [40 * mm, CONTENT_W - 40 * mm]),
            callout("note", "正在录制时修改", "短句模式下，相关静音参数的修改也从下一次录制生效。历史录音保留原检测结果；旧任务缺少新设置时按关闭处理，不补算历史。"),
            P("恢复设置会恢复进入任务时的录制策略与录制规则；静音参数使用检测页自己的恢复按钮。保存后核对界面显示，再开始下一次录制。"),
        ]
    if kind == "amplitude":
        return [
            *figure(CAP / "18-warning-confirmation.png", "幅值异常待确认区（模拟演示，RMS 下限设为 −10 dBFS）", max_h=40 * mm),
            P("开启检查后，软件按实际保存的单声道音频中被当前检测器识别为人声的区间计算，排除非人声区间。长静音不会拉低人声 RMS；界面上实时、平滑的监听电平不用于最终判定。"),
            table("提醒怎么判断", ["阶段", "含义"], [
                ["录制中声音偏小", "人声 200 毫秒窗口 RMS 持续低于下限达 300 毫秒时提示"],
                ["录制中声音过大", "人声峰值超过上限立即提示；等于阈值不触发超限"],
                ["结束后检查", "计算整句人声 RMS 和最大峰值；等于边界不算异常。短于实时窗口的人声也执行最终检查"],
                ["提醒保留", "即使最终整句指标正常，录制中出现过的幅值提醒仍保留"],
            ...
            P("无有效人声时沿用无声处理。检测器不识别朗读内容，噪声也可能被误判为人声；请结合试听和现场环境复核，不要只凭一个数值确认录音。"),
        ]
    if kind == "auto":
        return [
...
def build_story() -> list:
    ...
    s.append(P("标贝音频采集 0.2  ·  2026-09-07", "center_muted"))
    return s
