...
def build_story() -> list:
    ...
    s += [
        heading(1, "新建录制"),
        P("在任务列表点「新建录制」。分三步：导入脚本、选择设备和格式、命名并选择保存位置。创建后进入查看，此时声卡未打开。"),
    ]
    s.extend(figure(
        ANN / "setup.png",
        "新建录制",
        [
            ...
            "采集预设。",
            "输入设备。",
            "采集格式、采样率、输入通道。不支持的组合会标红，不能创建。",
            "静音阈值（RMS dBFS）和前后静音时长。",
            "当前组合是否可用。",
            "录制名称和保存位置。",
            "创建录制任务。",
        ],
    ...
    s.append(bullets([
        "采集预设：保存或套用本机常用的设备与格式组合。",
        "Windows 采集模式：独占，或系统混音。独占失败会报错，不会改成混音后仍显示独占。",
        "采集格式：16-bit PCM、24-bit PCM、32-bit Float。母带、试听和导出使用同一格式。",
    ...
    s.append(heading(2, "进入录制与环境检测"))
    s.append(P("查看时主按钮为「试听本句」，整页右下角为「进入录制」，其下为「退出查看」。点「进入录制」或任务列表的「录制」后打开声卡并开始写母轨。新建或恢复开卡后会做一次约 3 秒的环境噪声检测，期间保持安静。通过后本任务不再重复检测；未通过则降低环境噪声后点「重新检测」。未进入录制时声卡未打开，查看态显示该句波形或「本句尚无录音」，不把 Peak / RMS 显示成 −∞。"))
    ...
    s.append(P("未检测到语音就停止：取消本句，不生成可用录音。已开口但尾静音不够也可以结束，该次录音会标记为尾静音未达标。停句后可查看「首 / 尾」时长；「请等待朗读」是开口提示，不是首静音。检测栏中「停句后显示首尾检查」只控制是否显示该信息。"))
    s.append(heading(2, "状态显示"))
    s.extend(figure(FIG / "cue-colors.png", "状态色", max_h=88 * mm))
    s.append(heading(2, "暂停与退出"))
    s.append(P("右下角拆成两个动作。「暂停采集」停止声卡并封存母轨，但留在当前任务，可继续查看、导出或重新进入录制。「退出任务」才返回列表。当前句已有语音则先封闭为待确认；未开口则取消这次空录音。全部句子已处理时，主按钮为完成采集；确认后进入查看模式，不回列表。离开用「退出任务」或查看里的「退出查看」。出现故障时按界面提示安全结束。"))

    s += [
        heading(1, "右侧栏"),
        P("四个页签：监听、检测、任务、导出。出现故障或磁盘告警时会多出「问题」。"),
    ]
    s.append(two_figures(
        (ANN / "side-monitor.png", "监听"),
        (ANN / "side-detect.png", "检测"),
        "监听与检测",
    ))
    s.extend(figure(
    ...
    s.append(P("窗口内可调字号（22–72 px，默认 36）和可朗读时的正文颜色（默认绿色）。设置保存在本机。"))
    ...
    s.append(table(
        ...
        ["按键", "未在录", "录制中", "待确认"],
        [
            ["Space", "查看时试听；录制时开始；全部完成后结束采集", "结束本句", "确认"],
            ["R", "查看时进入录制；录制时重录", "无", "重录"],
            ["P", "试听", "无", "试听"],
            ["S", "跳过（需已进入录制）", "无", "跳过"],
            ["← / →", "上一句 / 下一句", "不可用", "上一句 / 下一句"],
