...
OUT = ROOT.parent.parent / 'output/pdf/标贝音频采集_功能更新说明_2026-09-07.pdf'
W = 174 * mm
ST = {}
...
def page(c, doc):
    ...
    c.drawString(18*mm,10*mm,'文档版本：2026-09-07　｜　截图为当前界面演示')
    c.drawRightString(192*mm,10*mm,f'{doc.page} / 3')

def story():
    s=[]
    s += section(1,'新增人声幅值检查','14-new-recording-policy.png','图 1　新建录制中的人声检查与短句录制设置',[
        '新建录制和已有任务均支持开启「人声幅值检查」，默认关闭。可设置人声 RMS 下限和 PEAK 上限，初始参考值分别为 −30、−3 dBFS。参考值并非验收标准，应根据项目要求及试录结果确定。',
        '系统基于实际保存的单声道音频，按检测器识别的人声区间计算指标，排除非人声区间。录制中，人声 200 毫秒窗口 RMS 持续低于下限达 300 毫秒时提示「声音偏小」；峰值超过上限时立即提示「声音过大」。录制结束后执行整句检查，阈值相等不判为异常。',
    ],51)
    ...
    s += section(3,'增加幅值异常录音确认保留','18-warning-confirmation.png','图 3　幅值异常后的试听、重录与确认保留操作区',[
        '录制结束后在正文上方固定区域显示整句人声 RMS、PEAK；异常以黄色图标、警告文字及本次阈值说明，不挤动正文或波形。录制中的异常提醒也会保留。短于实时检测窗口的人声也执行最终检查。采录人员可试听、重录，或在按项目要求核对后点击「确认保留并录下一句」；末句或关闭连续录制时，以当前按钮文案为准。',
        '人工保留记录随录音保存，重新打开任务后仍可查看。幅值提醒本身不阻止导出，音频中断、写盘失败等阻断问题仍须处理。重录保留历史版本，不会自动替换原合格录音。检测配置、指标、提醒、人工保留记录及结束原因随录音和导出元数据保存。',
        '截图采用模拟音频演示，RMS 下限临时设为 −10 dBFS 以触发偏小提醒，该值并非推荐阈值。',
    ],34)
    ...
    s += section(5,'支持已有任务保存录制设置','17-task-recording-policy.png','图 5　已有任务右侧「设置」中的配置保存入口',[
        ...
        '历史录音保留原检测结果，不补算或重新判定。旧任务缺少新增设置时，两个功能按关闭处理；历史录音缺少检测结果，表示当时未检查，不代表已达标。',
    ],83)
    s.append(Spacer(1,15))
