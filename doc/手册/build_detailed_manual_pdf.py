...
CAP = ROOT / "captures"
OUT = REPO / "output" / "pdf" / "标贝音频采集_详细操作手册_2026-09-01.pdf"
APP_VERSION = "0.2.0"
...
def cover(canv, doc) -> None:
    ...
    canv.drawRightString(b.PAGE_W - 22 * mm, 20 * mm, "文档版本 2026-09-01")
    canv.restoreState()
...
def body(canv, doc) -> None:
    ...
    canv.setFont("PF", 8)
    canv.drawString(b.LEFT, 6.2 * mm, f"{APP_VERSION} · 2026-09-01 · 采录人员版")
...
def story() -> list:
    ...
    page(s, 3, "新建录制", "先确认脚本与采集参数；检测策略只管理静音判定和检测规则。")
    s.extend(b.figure(CAP / "02-setup.png", "新建录制页面：预览为行内按钮，创建操作固定在底部", max_h=96 * mm))
    s.append(steps([
        "浏览并选择 CSV、TSV 或 TXT 脚本。",
        "需要核对时点击导入框下方的行内“查看预览”按钮。",
        "选择采集预设和输入设备，在采集参数区确认采集模式、输入通道、采样率和输出位深。",
        "输出位深默认 16-bit PCM；客户明确要求时可选 8-bit PCM、24-bit PCM 或 32-bit Float。",
        "按项目节奏选择连续录制方式；需要时展开检测策略，选择 VAD / 电平门并调整静音规则。",
        "检查任务名和保存位置；底部就绪状态与创建按钮始终可见。",
    ]))
    ...
    s.append(b.table("采集参数", ["项目", "建议", "注意"], [
        ...
        ["采集模式", "专业声卡优先 ASIO", "失败会明确报错，不静默降级"],
        ["输入通道", "按话筒物理接入选择", "选错可能只有底噪或无声"],
        ["采样率", "按客户要求；常用 48 kHz", "不再放在检测策略中"],
        ["输出位深", "默认 16-bit；可选 8/16/24/32", "决定母音频与导出 WAV"],
        ["驱动输入格式", "由软件自动选择", "普通任务设置不显示，实际值写入元数据"],
    ...
    s.append(steps([
        ...
        "环境检测完成后，试听面板先提供“跳过试听”和“开始 10 秒试听”。",
        "开始后右下角只保留常态可点击的“结束并跳过试听”；点击会结束测试录音并记录为已跳过。",
        "右上角 × 用于取消试听，只退出面板、不记录跳过；完成或明确跳过前不能开始正式录音。",
        "录满 10 秒后自动回放；回放出现后即可确认开始，无需等完整播放，也可直接跳过试听。",
    ]))
