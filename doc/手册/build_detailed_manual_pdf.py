...
def story() -> list:
    ...
    s.append(steps([
        ...
        "完成环境检测和 10 秒输入试听，再逐句执行“开始录音 - 完成本句 - 确认并录下一句”。",
    ...
    page(s, 3, "新建录制", "先确认脚本、采集预设、输入设备与通道；技术参数收在高级设置。")
    s.extend(b.figure(CAP / "02-setup.png", "新建录制页面：导入成功后可直接创建，需要时再查看预览", max_h=96 * mm))
    s.append(steps([
        "浏览并选择 CSV、TSV 或 TXT 脚本。",
        "选择采集预设、输入设备和输入通道，核对一行实际配置摘要。",
        "如需改动采样率或驱动输入格式，在高级设置中完成；不支持的组合会列出原因。",
    ...
    s.append(b.table("采集参数", ["项目", "建议", "注意"], [
        ...
        ["交付格式", "48 kHz / 16-bit PCM / 单声道", "本次试运行新任务统一使用"],
        ["驱动输入格式", "使用设备实际支持的 i16/i24/i32/f32", "与交付位深分开记录"],
    ...
    page(s, 6, "环境检测与输入试听", "先检查环境底噪，再用耳机确认实际输入；两者互不替代。")
    ...
    s.append(steps([
        ...
        "环境检测完成后朗读 10 秒测试语音，听完回放再确认开始；也可明确跳过。",
    ]))
    s.append(b.callout("warn", "不要把试听当成质量认证", "持续高噪声、底噪突变、爆音或无输入，应先排查麦克风、声卡、线材和系统占用；“已试听”只表示操作员听过本次输入。"))

    page(s, 7, "录制工作台怎么读", "左侧找句子，中间完成录制，右侧处理监听、检测、设置、任务、导出和问题。")
    ...
    page(s, 12, "检测与录制设置", "检测页只管理判定方式、静音阈值和时长；设置页管理连续录制、保护和提示。")
    s.append(b.two_figures(
        (CAP / "09-detection.jpg", "检测：只保留判定参数"),
        (CAP / "13-recording-settings.jpg", "设置：录制行为与提示"),
        "检测与本次录制设置",
    ))
    s.append(b.callout("note", "两个连续录制开关", "“确认后自动录下一句”是主开关；关闭后“标签变化时先暂停”会置灰但保留选择。摘要会直接说明当前是逐句暂停、同标签连续，还是跨标签连续。"))
    s.append(b.callout("tip", "设置作用范围", "右侧设置只影响当前任务；以后新任务的默认值在顶部“应用设置”中修改。真实缺帧和空音频不可交付等数据保护不能关闭。"))
    ...
    s.append(b.table("常见情况", ["现象", "可能原因", "处理"], [
        ...
        ["链路 warning 后需重录", "驱动报告了短暂抖动", "稍后补录受影响句，任务可继续"],
...
def concise_story() -> list:
    ...
    page(s, 3, "设备和环境检测", "设备与通道按实际接线选择；格式看实际配置摘要，检测不过就先解决现场问题。")
    s.append(b.two_figures(
    ...
    s.append(steps([
        ...
        "实际配置摘要应显示 48 kHz / 16-bit PCM / 单声道；驱动输入显示 i24/i32/f32 也不代表改了交付位深。",
        "环境检测时保持安静，不碰桌面、不敲键盘、不移动麦克风。",
        "随后朗读 10 秒测试语音并听完回放，确认设备、声道、底噪和爆音。",
    ...
    s.append(b.bullets([
        ...
        "设置：调整确认后连续录制、标签变化暂停、录制保护与提示；侧栏只影响当前任务。",
    ...
    page(s, 11, "应用设置、领读面板和快捷键", "应用设置管理以后新任务的默认值；领读面板只负责显示当前句和标签。")
    s.append(b.two_figures(
        (CAP / "11-settings.jpg", "应用设置"),
