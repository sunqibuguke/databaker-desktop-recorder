...
CAP = ROOT / "captures"
OUT = REPO / "output" / "pdf" / "标贝音频采集_详细操作手册_2026-08-27.pdf"
APP_VERSION = "0.2.0"
...
def cover(canv, doc) -> None:
    ...
    canv.drawRightString(b.PAGE_W - 22 * mm, 20 * mm, "文档版本 2026-08-27")
    canv.restoreState()
...
def body(canv, doc) -> None:
    ...
    canv.setFont("PF", 8)
    canv.drawString(b.LEFT, 6.2 * mm, f"{APP_VERSION} · 2026-08-27 · 采录人员版")
...
def story() -> list:
    ...
    s.append(b.table("快捷键", ["按键", "动作", "安全说明"], [
        ["Space", "开始 / 完成 / 确认并录下一句", "以主按钮当前文案为准"],
        ["R", "重录当前句", "处理后不自动开录下一句"],
    ...
    s.append(b.table("常见情况", ["现象", "可能原因", "处理"], [
        ...
        ["一句完成后停住", "待确认、输入不连续或重录待处理", "看主按钮和问题面板，不连续按 Space"],
...
def concise_story() -> list:
    ...
    s.append(b.callout("tip", "定位不会开录", "上一条、下一条、需重录定位都只会选中句子，不会突然开始录音。"))
    ...
    s.append(b.bullets([
        ...
        "出现检测异常时，母轨仍会优先保存；当前受影响的句子按提示重录。",
    ...
    s.append(b.table("常用快捷键", ["按键", "动作", "注意"], [
        ["Space", "开始 / 完成 / 确认并录下一句", "以当前主按钮文案为准"],
        ["R", "重录当前句", "处理后不自动录下一句"],
        ["P", "试听", "不能试听时按界面提示处理"],
        ["S", "跳过", "完整任务导出会被跳过句阻止"],
