...
位深选项控制实际写入 WAV 的编码，不是只写入元数据。硬件输入会优先使用驱动暴露的高精度采样格式，再转换为所选交付格式。如果声卡只提供低精度输入，选择更高交付位深不会凭空增加硬件有效精度。Windows WASAPI shared mode 下，可选采样率和样本格式表示 Windows 音频引擎接受的客户端交付格式，不证明声卡 ADC 以同样的原生采样率或位深工作，也不代表 bit-perfect；Windows 音频引擎可能重采样或格式转换。候选客户端格式还会受 `GetMixFormat` 有效精度上限约束；不会把 `IsFormatSupported` 接受格式转换误当成声卡精度升级。Windows 验收工具会分别记录输入样本格式与交付 WAV 位深，并按有效数字精度判定输入门槛（整数 `n` bit 按 `n`，`f32=24`，`f64=53`）。该数字样本精度不等于声卡 ADC ENOB，后者仍需根据硬件规格和专业测量归档。
