...
采集格式选项列出当前设备在所选后端、采样率和输入通道下真正能开的样本类型（`i16` / `i24` / `i32` / `f32`）。文件按同一规格写入：`i16` → 16-bit PCM，`i24` → 24-bit PCM，`i32` / `f32` → 32-bit Float。未指定采集格式时，引擎仍按交付位深自动挑选输入格式。Focusrite 等专业声卡优先使用厂商 ASIO 驱动，默认请求 512 帧缓冲区；ASIO 开流失败会明确报错，不会静默降级到 WASAPI/WDM。普通麦克风和未提供 ASIO 的 USB 设备继续使用 WASAPI 兼容路径。任务会分开记录后端、请求/实际缓冲区、驱动输入格式与交付 WAV 编码。Windows 验收工具按有效数字精度判定输入门槛（整数 `n` bit 按 `n`，`f32=24`，`f64=53`）。该数字样本精度不等于声卡 ADC ENOB，后者仍需根据硬件规格和专业测量归档。
