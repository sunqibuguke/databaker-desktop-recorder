...
位深选项控制实际写入 WAV 的编码，不是只写入元数据。硬件输入会优先使用驱动暴露的高精度采样格式，再转换为所选交付格式。如果声卡只提供低精度输入，选择更高交付位深不会凭空增加硬件有效精度。Windows 工位默认 WASAPI 独占开流，绕过系统混音器；独占失败会明确报错，不会静默降级成共享还显示独占。系统混音（shared）下可选采样率和样本格式只表示 Windows 音频引擎接受的客户端交付格式，不证明声卡 ADC 以同样的原生采样率或位深工作，也不代表 bit-perfect。共享路径的候选客户端格式仍受 `GetMixFormat` 有效精度上限约束。任务会分开记录开流模式、驱动输入格式与交付 WAV 编码。Windows 验收工具按有效数字精度判定输入门槛（整数 `n` bit 按 `n`，`f32=24`，`f64=53`）。该数字样本精度不等于声卡 ADC ENOB，后者仍需根据硬件规格和专业测量归档。
