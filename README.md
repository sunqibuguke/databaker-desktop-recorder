...
- 枚举系统识别的麦克风与外置声卡输入；新建录制时把采集模式、输入通道、采样率和输出位深集中在同一个“采集参数”区域。输出位深默认 `16-bit PCM`，可按客户交付要求选择 `8-bit PCM`、`16-bit PCM`、`24-bit PCM` 或 `32-bit Float`，母音频、试听切片与导出 WAV 保持同一 Mono 格式。
- 新建录制页只有表单内容滚动，创建按钮和就绪状态固定在底部；脚本导入后用紧凑的“查看预览”按钮核对条数和标签，不再占用整行状态卡。采样率不再放在“检测策略（高级）”中，检测策略只管理静音判定和检测规则。
- Windows 使用 CPAL 的 ASIO + WASAPI 双后端：专业声卡优先走厂商 ASIO 驱动，普通麦克风和通用 USB 设备保留 WASAPI；输入回调会申请 MMCSS 实时优先级，并对驱动包位置做连续性校验。
- 新建或恢复一次录制后会先按任务规则执行约 3 秒的环境噪声检测，再在正式录句前提供 10 秒输入试听。试听面板首次打开时可选择“跳过试听”或“开始 10 秒试听”；开始后右下角只保留常态可点击的“结束并跳过试听”，右上角关闭按钮用于取消试听且不记录为已跳过。输入试听与环境检测互不替代；同一次应用启动、相同采集配置不重复提示。
...
操作手册已同步到 2026-09-08：[使用手册](doc/手册/标贝音频采集_使用手册.pdf)与[详细操作手册](output/pdf/标贝音频采集_详细操作手册_2026-09-08.pdf)包含 16-bit samp 人声峰值检查（默认 3000～20000、可切换 dBFS）、短句自动结束、人工保留、旧任务兼容及授权异常封存说明。本次变更另见[功能更新说明](output/pdf/标贝音频采集_功能更新说明_2026-09-08.pdf)与[新增功能快速上手](output/pdf/标贝音频采集_新增功能快速上手_2026-09-08.pdf)。手册源码与截图复现步骤见 [手册维护说明](doc/手册/README.md)。

Windows 安装包会把验收工具、启动器和文档放在 `resources/acceptance/`，把标准使用手册和详细操作手册放在 `resources/manuals/`。工具覆盖 16/24/32-bit 短录、2–8 小时长稳、USB 拔出、专用测试卷磁盘保护，以及 `power-cut` → 重启 → `recover` 的真实断电两阶段门禁。生产 `power-cut` 只有在至少 1 小时样本已 committed、文件仍增长、尾差在预算内且 `overflow_samples=0` 时，才会把该零溢出事实写入 nonce armed 证据并提示断电；资格汇总会在 phase-1 报告、独立证据、会话内证据和 armed telemetry 之间交叉验证。`recover` 必须显式传入独立的 `--phase1-report`，并与会话内证据逐字段一致，同时匹配同一主机的新 boot、恢复前非终态和 `no_op=false`；正常 stopped 任务、仅杀进程、修改单份证据或丢失 armed committed 音频都不能 PASS。它还会严格检查 WAV 头部/EOF、分段编号、完整帧、物理样本水位、导出 status/metadata/CSV 一致性、overflow/fault marker 和引擎退出。显式的 `--test-only-power-cut` 短时回归只会生成 `TEST_ONLY_PASS` / `production_eligible=false`，不具备生产验收资格。Windows CI 还会静默安装实际 NSIS 产物、从安装目录运行验收启动器，并与安装包一起发布 `SHA256SUMS.txt`。完整命令和 PASS/FAIL 标准见 [Windows 外置声卡生产验收](doc/Windows外置声卡生产验收.md)。

当前交付仍属于小范围试运行，正式工位资格不是本版上线门槛；USB 物理插口也不会被绑定或比较。现场边界与回退条件见 [受控试运行说明](doc/受控试运行说明.md)。

### 推荐采集参数

- 默认：`48,000 Hz / 16-bit PCM / Mono`。客户明确指定时，在创建任务前选择对应采样率和输出位深。
- 输出位深可选 `8-bit PCM`、`16-bit PCM`、`24-bit PCM`、`32-bit Float`；该选项同时决定母音频和导出 WAV 的位深。8-bit 仅用于明确要求的兼容性交付，常规语音采集默认使用 16-bit 或项目指定规格。
- 多输入声卡会显示“输入 1、输入 2…”；软件从所选硬件通道采集并交付单声道 WAV。

普通任务设置不再显示“驱动输入格式”。软件会根据设备、后端、采样率、通道和所选输出位深自动选择能够满足精度要求的 `i16` / `i24` / `i32` / `f32` 输入表示，并把实际值写入任务元数据；操作员只需核对客户要求的采样率和输出位深。Focusrite 等专业声卡优先使用厂商 ASIO 驱动，默认请求 512 帧缓冲区；ASIO 开流失败会明确报错，不会静默降级到 WASAPI/WDM。普通麦克风和未提供 ASIO 的 USB 设备继续使用 WASAPI 兼容路径。Windows 验收工具按有效数字精度判定输入门槛（整数 `n` bit 按 `n`，`f32=24`，`f64=53`）。该数字样本精度不等于声卡 ADC ENOB，后者仍需根据硬件规格和专业测量归档。
