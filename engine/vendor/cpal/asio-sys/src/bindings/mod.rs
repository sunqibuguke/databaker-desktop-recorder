...
pub struct CallbackInfo {
    ...
    ///
    /// This is normalized to zero when [`Self::system_time_valid`] is false.
    /// Consumers must use the validity field rather than inferring validity
    /// from this numeric value because zero is also a valid time at startup.
    pub system_time: u64,
    /// Whether the driver set `kSystemTimeValid` for this callback.
    pub system_time_valid: bool,
    /// Sample position at the start of this buffer period.
    ///
    /// This is normalized to zero when [`Self::sample_position_valid`] is
    /// false. Consumers must not use it for continuity checks unless the
    /// validity field is true.
    pub sample_position: u64,
    /// Whether the driver set `kSamplePositionValid` for this callback.
    pub sample_position_valid: bool,
    /// Raw `ASIOTimeInfo::flags` supplied by the driver.
    ///
    /// The explicit validity booleans above should be preferred when reading
    /// the corresponding numeric fields.
    pub time_info_flags: i32,
...
/// Convert an `ASIOSamples` position (high and low 32-bit halves) to a `u64`.
#[inline]
fn asio_samples_to_u64(samples: ai::ASIOSamples) -> u64 {
    (samples.hi as u64) << 32 | samples.lo as u64
}

const SYSTEM_TIME_VALID_FLAG: i32 = 1;
const SAMPLE_POSITION_VALID_FLAG: i32 = 1 << 1;

/// Construct callback metadata at the FFI boundary so invalid driver fields
/// cannot accidentally escape as plausible timestamps or sample positions.
#[inline]
fn callback_info_from_raw(
    buffer_index: i32,
    system_time: u64,
    sample_position: u64,
    time_info_flags: i32,
    ...
    let system_time_valid = time_info_flags & SYSTEM_TIME_VALID_FLAG != 0;
    let sample_position_valid = time_info_flags & SAMPLE_POSITION_VALID_FLAG != 0;
    CallbackInfo {
        buffer_index,
        system_time: if system_time_valid { system_time } else { 0 },
        system_time_valid,
        sample_position: if sample_position_valid {
            sample_position
        } else {
            0
        },
        sample_position_valid,
        time_info_flags,
        callback_flag,
    }
}
...
extern "C" fn buffer_switch_time_info(
    ...
    let time_info_flags = asio_time.time_info.flags;
    let callback_info = callback_info_from_raw(
        double_buffer_index,
        asio_timestamp_to_nanos(asio_time.time_info.system_time),
        asio_samples_to_u64(asio_time.time_info.sample_position),
        time_info_flags,
        callback_flag,
    );
...
// The non-Windows build uses documentation stubs rather than the SDK's C ABI,
// so layout equivalence is meaningful only on the platform that loads ASIO.
#[cfg(target_os = "windows")]
#[test]
fn check_type_sizes() {
    assert_eq!(
...
#[test]
fn callback_info_exposes_valid_driver_timing_metadata() {
    let flags = SYSTEM_TIME_VALID_FLAG | SAMPLE_POSITION_VALID_FLAG;
    let info = callback_info_from_raw(1, 123_456, 654_321, flags, 7);

    assert_eq!(info.buffer_index, 1);
    assert_eq!(info.system_time, 123_456);
    assert!(info.system_time_valid);
    assert_eq!(info.sample_position, 654_321);
    assert!(info.sample_position_valid);
    assert_eq!(info.time_info_flags, flags);
    assert_eq!(info.callback_flag, 7);
}

#[test]
fn callback_info_zeroes_driver_values_without_validity_flags() {
    let unrelated_flag = 1 << 4;
    let info = callback_info_from_raw(0, u64::MAX, u64::MAX - 1, unrelated_flag, 3);

    assert_eq!(info.system_time, 0);
    assert!(!info.system_time_valid);
    assert_eq!(info.sample_position, 0);
    assert!(!info.sample_position_valid);
    assert_eq!(info.time_info_flags, unrelated_flag);
}

#[test]
fn callback_info_honors_timing_validity_flags_independently() {
    let time_only = callback_info_from_raw(0, 11, 22, SYSTEM_TIME_VALID_FLAG, 0);
    assert_eq!(time_only.system_time, 11);
    assert!(time_only.system_time_valid);
    assert_eq!(time_only.sample_position, 0);
    assert!(!time_only.sample_position_valid);

    let position_only = callback_info_from_raw(0, 11, 22, SAMPLE_POSITION_VALID_FLAG, 0);
    assert_eq!(position_only.system_time, 0);
    assert!(!position_only.system_time_valid);
    assert_eq!(position_only.sample_position, 22);
    assert!(position_only.sample_position_valid);
}

#[test]
fn asio_samples_to_u64_preserves_both_halves() {
    assert_eq!(
        asio_samples_to_u64(ai::ASIOSamples {
            hi: 0x1234_5678,
            lo: 0x90ab_cdef,
        }),
