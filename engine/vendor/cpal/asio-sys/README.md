...
### Callback timing metadata

`CallbackInfo` exposes the driver's raw `ASIOTimeInfo::flags` together with
explicit `system_time_valid` and `sample_position_valid` fields. Always check
the matching validity field before using `system_time` or `sample_position`.
Invalid numeric fields are normalized to zero at the FFI boundary, but zero by
itself does not imply invalidity because it can also be a valid stream-start
value.
