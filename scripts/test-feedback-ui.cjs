...
assert.match(recorder, /const isCurrent = index === currentIndex/);
assert.match(recorder, /className=\{`professional-item\$\{isCurrent \? ' current-item' : ''\}\$\{isRecordingCurrent \? ' recording-item' : ''\}/);
assert.match(recorder, /className=\{`item-current-flag\$\{isRecordingCurrent \? ' recording' : ''\}`\}/);
assert.match(css, /\.professional-item\.active,[\s\S]*?\.professional-item\.current-item\s*\{[^}]*border-left-color:\s*var\(--accent\)[^}]*outline:[^}]*linear-gradient/);
assert.match(css, /\.professional-item\.current-item\.recording-item\s*\{[^}]*border-left-color:\s*var\(--record\)[^}]*linear-gradient/);
