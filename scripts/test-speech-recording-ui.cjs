...
async function main() {
  ...
  try {
    ...
    await expect(page.locator('.speech-quality-banner, .speech-review.has-warning')).toContainText('声音过大', { timeout: 10_000 });
    assert.equal(await page.evaluate(() => window.__speechPrompter.qualityWarning), '声音过大');
    ...
    await expect(page.getByTestId('speech-quality-result')).toContainText('声音过大');
    await expect(page.getByTestId('speech-quality-result')).toContainText('超过本次上限 8000 samp');
    ...
    await expect(page.getByTestId('speech-quality-result')).toContainText('超过本次上限 8000 samp');
    await expect(page.getByTestId('speech-quality-result')).not.toContainText('本次上限 20000');
    ...
    assert.equal(finished.snapshot.items[1].attempts[0].speech_quality.policy.peak_samp.max, 8000);
    assert.equal(finished.snapshot.items[0].attempts[0].speech_quality.policy.peak_samp.max, 8000);
    ...
    await expect(page.getByLabel('短句自动结束', { exact: true })).toBeChecked();
    await expect(page.getByLabel('人声峰值上限（samp）', { exact: true })).toHaveValue('8000');
